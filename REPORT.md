# قهوینو — گزارش نهایی پروژه / Final Report

**پروژه:** فروشگاه آنلاین قهوهٔ تخصصی «قهوینو» (GHAHVINO Roasters)
...
| کد تخفیف (`GHAHVINO10`، `FILTER20`) | اعتبارسنجی محلی | اعتبارسنجی سرور |
...
> **به‌روزرسانی — ری‌برند به «قهوینو»:** این گزارش مربوط به فاز ساخت فروشگاه است. تغییر
> هویت به «قهوینو» (`https://ghahvino.ir`)، منبع واحد `src/config/site.ts`، متادیتای هر
> صفحه، `sitemap.xml`/`robots.txt`، داده ساختاریافته، حذف داده‌های تأییدنشده و نتایج
> Lighthouse/ریسپانسیو در [`RESPONSIVE_SEO_AUDIT.md`](./RESPONSIVE_SEO_AUDIT.md) آمده است.
> کلیدهای `localStorage` عمداً با پیشوند قدیمی `darband.*` باقی مانده‌اند تا سبد خرید،
> علاقه‌مندی‌ها و نشست کاربران فعلی از بین نرود (دلیل کامل در همان سند).
