...
| مسیر | محتوا |
|---|---|
| `/auth` | ورود/ثبت‌نام یکپارچه؛ لوگو، عنوان، توضیح حریم خصوصی، ورودی موبایل ۱۱ رقمی (`09…`، بدون پیشوند کشور)، CTA ارسال کد، لینک بازگشت به فروشگاه، قوانین/حریم خصوصی، ستون ادیتوریال تصویری در دسکتاپ |
...
- شماره‌ها پیش از ذخیره به فرمت Canonical **`09121234567`** (۱۱ رقمی) نرمال می‌شوند (`auth.utils.ts`)؛ نمایش UI فارسی و خوانا است و هیچ‌جا پیشوند `+98` دیده نمی‌شود.
...
| فرمت پذیرفته‌شده | فقط **۱۱ رقم با شروع `09`** → `09121234567` (ارقام فارسی/عربی، فاصله و خط تیره پاک‌سازی می‌شوند؛ `+98`، `0098` و شکل ۱۰ رقمی رد می‌شوند) |
